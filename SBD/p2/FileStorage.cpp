...
BTreeHeader FileStorage::read_header(Metrics *m) {
    if(m) m->inc_header_reads();
    int rv;
    rv = fseek(file.get(), 0, SEEK_SET);
    assert(rv > -1);
    BTreeHeader hdr;
...
void FileStorage::write_header(BTreeHeader header, Metrics *m) {
    if(m) m->inc_header_writes();
    int rv;
    rv = fseek(file.get(), 0, SEEK_SET);
    assert(rv > -1);
